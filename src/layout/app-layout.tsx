...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['event-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AdminInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/admins` : null),
    () => getAdmins({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Admin',
      path: isTenantUser ? `/admins/view/${data?.[0]?.id}` : '/admins',
      entity: 'admin',
